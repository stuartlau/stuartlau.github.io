...
def check_ffmpeg():
    ...
    if shutil.which("ffmpeg") is None:
        print(
            "Error: ffmpeg is not installed. Please install it using 'brew install ffmpeg'"
        )
        sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert MOV files to MP4 to save space."
    )
    parser.add_argument(
        "path", help="File or directory path to process", default=".", nargs="?"
    )
    ...
    parser.add_argument(
        ...
        action="store_false",
        dest="delete",
        help="Keep original MOV files (default is to delete)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=23,
        help="CRF value for quality (default: 23). Lower is better quality, higher is smaller size.",
    )
            ...
            for root, dirs, files in os.walk(target_path):
                for file in files:
                    if file.lower().endswith(".mov"):
                        convert_mov_to_mp4(
                            os.path.join(root, file), args.delete, args.crf
                        )
        else:
            for file in os.listdir(target_path):
                if file.lower().endswith(".mov"):
                    convert_mov_to_mp4(
                        os.path.join(target_path, file), args.delete, args.crf
                    )
    else:
