    ...
    try:
        ...
        if not details.get("genres") or not details.get("platforms"):
            ...
            release_match = re.search(
                r"发行日期[：:]\s*(\d{4}[-/年]\d{1,2}[-/月]\d{1,2})", page_text
            )
...
def scrape_games(cookie, max_pages=None):
    ...
    existing_ids = {
        item.get("game_id") for item in existing_items if item.get("game_id")
    }
    ...
    while page < max_page_limit:
        ...
        url = (
            f"{BASE_URL}?action=collect"
            if page == 0
            else f"{BASE_URL}?action=collect&start={page * 15}"
        )
        ...
        try:
            ...
            for item in items:
                try:
                    ...
                    existing_item = next(
                        (i for i in existing_items if i.get("game_id") == game_id), None
                    )

                    if existing_item:
                        missing_fields = (
                            not existing_item.get("douban_rating")
                            or not existing_item.get("release_date")
                            or not existing_item.get("genres")
                            or not existing_item.get("platforms")
                        )
                        if missing_fields:
                            print(
                                f"  * Updating metadata for: {game_data.get('title')}"
                            )
