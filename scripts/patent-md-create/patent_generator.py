    ...
    try:
        ...
        if not data["events"]:
            for section in soup.find_all(
                ["section", "div"], class_=lambda x: x and "patent-result" in x
            ):
                h2 = section.find("h2")
        ...
        for d, desc in data["events"]:
            if "granted" in desc.lower():
                grant_date = d
            if f"Publication of {patent_id}" in desc or (
                f"Publication of" in desc and patent_id[2:11] in desc
            ):
                pub_date = d
        ...
        if patent_id.endswith("B"):
            data["date"] = (
                grant_date if grant_date else (pub_date if pub_date else meta_date_val)
            )
        else:
    ...
    for ef in existing_files:
        ...
        if f"-{data['id']}.md" in ef:
            ...
            if ef.endswith("A.md") and filename.endswith("B.md"):
                ...
                print(
                    f"Found grant version {ef}, skipping application version {filename}."
                )
                return
