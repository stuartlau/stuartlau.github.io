...
def get_subject_details(url, cookie):
    ...
    try:
        ...
        details["directors"] = [
            d.strip()
            for d in (get_attrs_by_label("导演") or "").split("/")
            if d.strip()
        ]
        details["writers"] = [
            w.strip()
            for w in (get_attrs_by_label("编剧") or "").split("/")
            if w.strip()
        ]
        details["cast"] = [
            c.strip()
            for c in (get_attrs_by_label("主演") or "").split("/")
            if c.strip()
        ]

        # Genres from property tags
        details["genres"] = [
            g.get_text() for g in soup.select('span[property="v:genre"]')
        ]
...
def scrape_movies(cookie, max_pages=None):
    ...
    existing_ids = {
        item.get("movie_id") for item in existing_items if item.get("movie_id")
    }
        ...
        try:
            ...
            for item in items:
                try:
                    ...
                    existing_item = next(
                        (i for i in existing_items if i.get("movie_id") == movie_id),
                        None,
                    )
                    ...
                    if existing_item:
                        ...
                        missing_fields = (
                            not existing_item.get("douban_rating")
                            or not existing_item.get("pub_date")
                            or not existing_item.get("genres")
                            or not existing_item.get("directors")
                        )
                        if missing_fields:
                            print(
                                f"  * Updating metadata for: {movie_data.get('title')}"
                            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--max-pages", type=int, help="Maximum number of pages to fetch (default: 100)"
    )

    # Optional cookie argument, defaults to reading from file
    parser.add_argument(
        "--cookie", type=str, help="Douban cookie string (overrides ~/.douban.cookie)"
    )
