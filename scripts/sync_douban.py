...
def update_summary():
    ...
    for year_file in DATA_DIR.glob("*.json"):
        ...
        if year.isdigit() and len(year) == 4:
            ...
            ratings = [
                item.get("my_rating", 0) for item in data if item.get("my_rating")
            ]
...
def main():
    ...
    parser.add_argument(
        "--year", type=str, default="all", help='Year to sync (e.g., 2026), or "all"'
    )
    parser.add_argument(
        "--cookie", type=str, default=None, help="Douban cookie for authentication"
    )
