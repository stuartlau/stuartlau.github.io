...
def fetch_all_albums(cookie=None):
    ...
    while True:
        ...
        try:
            ...
            if not album_items:
                ...
                for link in album_links:
                    ...
                    if match:
                        ...
                        title_el = (
                            parent.find(
                                "a", href=True, class_=lambda x: x != "album_photo"
                            )
                            if parent
                            else None
                        )
            ...
            if len(albums) == 0 or (
                page > 1 and len(albums) == len(set(a["id"] for a in albums))
            ):
...
def main():
    parser = argparse.ArgumentParser(
        description="Auto-match Douban albums to travel posts"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Show matches without making changes"
    )
    parser.add_argument(
        "--fetch-albums", action="store_true", help="Fetch and cache album list"
    )
    parser.add_argument(
    ...
    if not cookie:
        print(
            "Warning: No cookie provided. Requests may fail due to Douban anti-crawl."
        )
