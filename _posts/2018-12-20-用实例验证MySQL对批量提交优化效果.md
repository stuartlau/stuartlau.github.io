...
```
### 总结
可以通过上面的BINLOG和general_log看到第一个SQL的提交方式MySQL内部使用了事务来处理，并且合并成了一条SQL。
默认情况下**SELECT @@session.autocommit**返回值为1，即开启了自动提交，而由于我们使用的是writer库，所以**select @@session.tx_read_only**返回值为0，即不是read_only模式。
...
- 打开general_log开关：**SET GLOBAL general_log=ON**
- 查看general_log信息：**SHOW VARIABLES LIKE 'general%'**
