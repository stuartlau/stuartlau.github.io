...
一个简单的使用示意图及解释(摘自[xiaobaoqiu blog](http://xiaobaoqiu.github.io/blog/2015/07/02/ratelimiter/))
，例子中SmoothBursty的QPS=4:

下图描述了一个场景

> (1).开始，t=0，这时候storedPermits=0，此时请求1个令牌，可以透支，等待时间=0，下一次可执行时间1ms以后；
>
> (2).等待时间到，t=1，这时候storedPermits=3（已经放入4个并且除去透支的1个），请求3个令牌，可以拿去（此时storedPermits=3），等待时间=0
，下一次可执行时间为1s以后，会产生4个新的；
>
> (3).等待时间到，t=2，这时候storedPermits=4（又放入了4个），请求10个令牌，等待时间=0（又透支），超前使用了2个令牌，下一次可执行时间为2s以后，会产生8个；
>
> (4).等待时间到，t=3，这时候storedPermits=0（因为此时还能放入两个，所以半秒放入1个），请求1个令牌，等待时间=0.5（等待半秒就返回了）；
...
```
可以看到，平稳的间隔时间是250000微妙=250ms，maxPermits就是QPS的值4。
![BurstyRateLimiter.png](http://xiaobaoqiu.github.io/images/guava/BurstyRateLimiter.png)
...
![](http://xiaobaoqiu.github.io/images/guava/SmoothWarmingUp.png)
## References
