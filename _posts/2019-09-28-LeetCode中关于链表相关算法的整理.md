...
public static ListNode reverseListByInsert(ListNode listNode) {
    ...
    // 而且需要将p节点插入到新的链表中
    while(p != null) {
        // 先保存待插入新链表的节点p之后的链表到tempList临时节点，用于下一轮遍历使用
...
这里很关键的一点是 `i += step * 2` ，保证了每一轮的对比中都会让两个新的没有对比过的元素进行合并，即 `i` 和 `i + step` ，注意每一轮 `step` 也是增倍的 
`step *= 2` ，因为每次待合并的两个节点的数据都是在和其之后 `step` 位置节点进行合并后的结果，所以 `step` 下一轮要跳过 `2` 倍的自己。
...
### 双向链表
#### 实现LRU
> 基于双向链表可以实现对元素的快速移动和收尾元素的快速定位，可以用来实现LRU算法。
>
> 将最近访问的元素移动到链表的头部，将超过大小的数据从尾部移除，将更新过的数据从原链表中删除后再插入到链表头部。

```java
private final int capacity;
private final Map<Integer, Node> cache; // 使用
