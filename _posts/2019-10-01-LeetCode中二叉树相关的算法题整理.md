...
```
### 树上最长单色路径
这道题不是LC上的，但是也很有特色，乍一看跟最大深度这道题差不多，但其实没有意识到一个问题就是路径并不一定是从父节点到子节点的，
也可能是从子节点到父节点再到子节点。

所以区别就是深度是只要加上左右孩子的深度中大的一个再加上本层即可，而这里是需要加上左右两边的有效值再加上本parent节点1。
...
```
### 判断是否为平衡树
> 任何两个子树的高度相差不大于1则为不平衡树，所以需要统计每一层的高度，叶子的高度为1
，依次向根移动一层高度加1，所以当前层的高度为1+左右节点的最大高度。注意用递归实现时，如果一旦在任意层发现了这样的不平衡条件，则直接向上层返回，上层根据该值继续想上层返回。
这样难免再靠近叶子节点的层级发现时还需要完成层层出栈的操作，极端情况下效率比较差。但是如果采用迭代的方式，从根往下计算又会存在重复计算的问题。
...
```
> 上面的算法充分利用了求树高度时可以计算二叉树是否是平衡二叉树的特性，避免了从顶部逐步向下计算导致的重复计算问题。时间复杂度O(N)。
...
>
> 使用递归的思路，从叶子节点开始依次向上计算，逐层统计并返回给上一层本层的结果。
