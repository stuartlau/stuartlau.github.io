...
```
### 归并排序
> 归并字面上的意思是「合并」，归并算法的核心思想是「分治法」，就是将一个数组一刀切两半，递归切，直到切成单个元素，然后重新组装合并，单个元素合并成小数组，两个小数组合并成大数组，直到最终合并完成，排序完毕。

```java
public static void sort(int[] arr) {
        int[] tempArr = new int[arr.length]; // 临时变量
...
private static void merge(int[] arr,  int[] tempArr,  int startIndex,  int middleIndex,  int endIndex) {
    // 注意mergeq前middleIndex的左半部分和有右部分已经是排好序的
    
    // 复制要合并的数据，注意startIndex和endIndex每次的范围都是缩小的
    // 只将待合并区间的数据复制到临时数组即可
    ...
    // 一共要复制endIndex-startIndex次数据
    for (int k = startIndex; k <= endIndex; k++) {
        if(leftHead > middleIndex){
            // 如果左边的首位下标大于中部下标，证明左边的数据已经排完了，此时只需要继续按顺序复制右侧的剩余部分
            arr[k] = tempArr[leftHead++];
        } else if (rightHead > endIndex){
            //如果右边的首位下标大于了数组长度，证明右边的数据已经排完了，此时只需要继续按顺序复制左侧的剩余部分
            arr[k] = tempArr[leftHead++];
...
> 归并排序是将一个长的序列递归切割成短的序列，直到序列元素为一个，然后在逐步这些一个元素的序列为有序序列，并依次合并出越来越长的序列。
>
> 从很多个各自包含一个元素的序列出发，通过几遍归并得到最终的有序序列，这其中需要做许多函数调用工作。与这几个元素做简单插入排序相比，归并排序消耗时间会更多。
