...
- 后者两个每轮的比较次数是`依次减少`的，并且比较是`从前往后`依次比较，每一轮的结果保证此轮所有待排序元素「之外的」数据是排好序的，
...
>
> 直到最后pivot两侧的数组都是单个元素时，整个数组也就成了有序的序列。

快速排序使用「分治法」，其中使用了「递归」的思想：
- 跳出条件是 `endIndex <= startIndex`，此时已经无需再比较，因为待排序集合只有一个元素了
...
void QuickSort(int *arr, int low, int high)
{
    if (low < high)
    {
        ...
        /*将pivot值赋值到i位置，此时i==j，且arr[i]==arr[i+1]，i位置用来存放pivot*/
        arr[i] = k; 
...
private static void mergeSort0(int[] arr, int[] tempArr, int startIndex, int endIndex){
    ...
    // conquer
    // 注意merge前[startIndex,middleIndex]和[middleIndex+1,endIndex]两部分已经分别由下一层递归排好序了
...
private static void merge(int[] arr,  int[] tempArr,  int startIndex,  int middleIndex,  int endIndex) {    
    ...
    // 只将待合并区间的数据复制到临时数组即可，其他区域由上一层merge来处理
    ...
    // 为待排序的index范围内从头到尾依次从两个子数组中寻找最小的值
...
属于对数时间 `O(log n)` ，合起来等于 `O(log2n)` ，也就是说，总的时间复杂度为 `O(nlogn)` 。
